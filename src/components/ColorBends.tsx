...
export interface ColorBendsProps {
  ...
  intensity?: number;
  bandWidth?: number;
  /** `speed`에 곱해 호버 시 목표 속도로 쓰입니다. 생략 시 호버로 속도 변경 없음. */
...
export default function ColorBends({
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      const hoverOn = hoverEnabledRef.current && pointerOverRef.current;
      const targetSpeed = hoverOn ? base * hoverMultRef.current : base;
      const curSp = material.uniforms.uSpeed.value;
      if (hoverEnabledRef.current) {
        const k = Math.min(1, dt * hoverSmoothRef.current);
        material.uniforms.uSpeed.value = curSp + (targetSpeed - curSp) * k;
      } else {
        material.uniforms.uSpeed.value = base;
      }
  ...
  useEffect(() => {
    ...
    hoverMultRef.current = hoverSpeedMultiplier ?? 1.5;
    hoverSmoothRef.current = hoverSpeedSmooth;
    hoverEnabledRef.current = hoverSpeedMultiplier !== undefined;
    if (!hoverEnabledRef.current || !pointerOverRef.current) {
